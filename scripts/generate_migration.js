...
Object.entries(INGREDIENT_DB).forEach(([id, names]) => {
    const aliasesArray = aliasesById[id] ? Array.from(aliasesById[id]) : [];
    // Escape single quotes
    const escape = (str) => str.replace(/'/g, "''");

    const aliasesSql = aliasesArray.length > 0
        ? `ARRAY['${aliasesArray.map(escape).join("', '")}']`
        : "ARRAY[]::text[]";

    // We assume default category 'misc' if not specific, 
    // but better to fetch existing or guess based on normalization logic...
    // Actually, normalization.ts comments have categories! 
    // I can't easily extract them here. 
    // I'll leave category AS IS if updating, or default to 'mixer' for insert.
    // The previous SQLs already created most ingredients with categories.
    // We should use ON CONFLICT DO UPDATE to set aliases.
...
console.log(sql);
fs.writeFileSync(path.join(__dirname, '../migrate_ingredients_v1_1.sql'), sql);
console.log("Migration script generated at migrate_ingredients_v1_1.sql");
